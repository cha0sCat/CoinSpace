...
export default class BitcoinWallet extends CsWallet {
  ...
  async load() {
    ...
    try {
      ...
      const { unspents } = await bip32.load();
  ...
  async _loadFeeRates() {
    const fees = await this.requestNode({
      method: 'GET',
      url: 'fee-estimates',
    });

    this.#feeRates.clear();
    const minimum = Math.max(1, Math.ceil(fees['144'] || fees['72'] || 1));
    const normal = Math.max(1, Math.ceil(fees['6'] || fees['3'] || fees['2'] || minimum));
    const fastest = Math.max(normal, Math.ceil(fees['1'] || fees['2'] || normal));
    this.#feeRates.set(BitcoinWallet.FEE_RATE_MINIMUM, BigInt(minimum));
    this.#feeRates.set(BitcoinWallet.FEE_RATE_DEFAULT, BigInt(normal));
    this.#feeRates.set(BitcoinWallet.FEE_RATE_FASTEST, BigInt(fastest));
  }
  ...
  async inspectAddressActivity() {
    ...
    const { active } = await bip32.load({ includeUnspents: false });
    return {
  ...
  #getKnownAddresses() {
    ...
    this.#accounts.forEach((account) => {
      addresses.push(...account.addresses, ...account.changeAddresses);
    });
